#include "../include/unfolding_matrix.cuh"
...
Matrix UnfoldingMatrix::GetRows(const std::vector<size_t> &I) const {
    size_t rows = I.size();
...
Matrix UnfoldingMatrix::GetCols(const std::vector<size_t>& J) const {
    size_t cols = J.size();
...
Matrix UnfoldingMatrix::GetMaxvol(const std::vector<size_t> &I,
